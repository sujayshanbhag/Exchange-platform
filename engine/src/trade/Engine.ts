...
import { CANCEL_ORDER, CREATE_ORDER, GET_BALANCE, GET_DEPTH, GET_OPEN_ORDERS, GET_TICKER, MessageFromApi, OFF_RAMP, ON_RAMP } from "../types/fromApi";
...
export class Engine {
    ...
    process({ message, clientId }: { message: MessageFromApi, clientId: string }) {
        switch (message.type) {
            ...
            case ON_RAMP: {
                ...
                RedisManager.getInstance().sendToApi(clientId, {
                    ...
                    payload: {
                        ...
                        txnId: txnId,
                        newBalance: this.balances.get(userId)?.[BASE_CURRENCY].available.toString()
                    }
                });
                break;
            }
            case OFF_RAMP: {
                ...
                const amount = Number(message.data.amount);
                const txnId = message.data.txnId;
                this.offRamp(userId, amount);
                RedisManager.getInstance().sendToApi(clientId, {
                    type: "OFF_RAMP_CONFIRMED",
                    payload: {
                        userId: userId,
                        amount: amount.toString(),
                        txnId: txnId,
            ...
            case GET_TICKER:
                RedisManager.getInstance().sendToApi(clientId, {
                    type: "TICKER",
                    payload: this.orderbooks.map(orderbook => ({
                            price: orderbook.currentPrice.toString(),
            ...
            case GET_BALANCE: 
                ...
                RedisManager.getInstance().sendToApi(clientId, {
                    type: "BALANCE",
                    payload: {
                        balance: this.balances.get(userId)!![BASE_CURRENCY].available.toString(),
                    }
                });
                break;
        }
    ...
    offRamp(userId: string, amount: number) {
    ...
    createOrder(market: string, price: string, quantity: string, side: "buy" | "sell", userId: string) {
        ...
        this.publishWsPriceUpdated(orderbook);
        return { executedQty, fills, orderId: order.orderId };
    }

    publishWsPriceUpdated(orderbook: Orderbook) {
        RedisManager.getInstance().publishMessage(`ticker.${orderbook.ticker()}`, {
            stream: `ticker.${orderbook.ticker()}`,
            data: {
                p: orderbook.currentPrice.toFixed(2).toString(),
                s: `${orderbook.ticker()}`,
                e: "ticker"
            }
        })
    ...
    publisWsDepthUpdates(fills: Fill[], price: string, side: "buy" | "sell", market: string) {
        ...
        if (side === "sell") {
            const updatedBids: [string, string][] = fills.map(f =>
                depth?.bids.find(x => x[0] === f.price) || [f.price, "0"]
            );
